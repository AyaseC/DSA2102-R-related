...
```{r echo=FALSE}
library(readxl)
bra <- read_excel("../data/BRA-BEL-FIFA-2018.xlsx", sheet=1)
bel <- read_excel("../data/BRA-BEL-FIFA-2018.xlsx", sheet=2)
fra <- read_excel("../data/FRA-ARG-FIFA-2018.xlsx", sheet=1)
arg <- read_excel("../data/FRA-ARG-FIFA-2018.xlsx", sheet=2)
passes <- function(x){
  no <- which(!is.na(x))
  return (x[,no])
}
...
```{r echo=FALSE}
cdnow <- read.csv("../data/CDNOW2.csv")
library(tidyverse)
library(dplyr)
